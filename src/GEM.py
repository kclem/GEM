...
class GEM:
            ...
            with tarfile.open(gemfile_location,"r:gz") as tar_handle:
                ...
                if "meta.json" in member_names:
                    ...
                    try:
                        ...
                        raise GEM_Error("Corrupted GEM file. Metadata file cannot be read")
    ...
    def create_html(self,output_file=None):
        ''' prints html version to view GEM contents'''
        html_path = self._gemfile_location + ".html"
...
<html lang="en">
  <head>
    <script type='text/javascript' src="https://raw.githubusercontent.com/caldwell/renderjson/master/renderjson.js"></script>
    <script>
...
if (define) define({renderjson:renderjson})
    ...
    <title>"""+self._gemfile_location+"""</title>
  </head>
  <body>
  <div class='container'>
      <div class='card my-3'>
          ...
          <div class='card-body'>
          <div id='metadata_json'></div>
          </div>
      </div>
  <script>
      document.getElementById("metadata_json").appendChild(
                  renderjson("""+json.dumps(self.metadata)+""")
                      );
      </script>""")
            html_out.write('<div class="card my-3"><div class="card-header"><h1>Data files</h1></div><div class="card-body"><ul>')
            files = self.get_files()
            file_count = len(files)
            if file_count > 0:
                for f in files:
                    html_out.write('<li><a href="">'+f+'</a></li>')
                html_out.write('</ul>')
